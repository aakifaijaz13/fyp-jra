...
var configuration = {
    widgetId: "336670706656313933393532",
    tokenAuth: "399001T7fgvbmLi648c3e92P1",
    identifier: document.getElementById('MobileNumber').value,
    success: (data) => {
        number_verify = true;
        if (email_verify && number_verify) {
            document.getElementById('submit_btn').disabled = false;
...
document.getElementById('verify_number').addEventListener('click', function () {
    initSendOTP(configuration);
});

document.getElementsById('MobileNumber').addEventListener('input', () => {
    number_verify = false;
    document.getElementById('submit_btn').disabled = true;
});